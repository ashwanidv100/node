...
static void dispose_inspector(uv_handle_t* handle) {
  inspector_socket_t* inspector = (inspector_socket_t*) handle->data;
  inspector_cb close =
...
static void close_connection(inspector_socket_t* inspector) {
  uv_handle_t* socket = (uv_handle_t*) &inspector->client;
...
static int write_to_client(inspector_socket_t* inspector,
  ...
  uv_stream_t* stream = (uv_stream_t*) &inspector->client;
...
static void encode_frame_hybi17(const char* message,
  ...
  if (masking_key != 0) {
    const char* mask_bytes = (char*) &masking_key;
  ...
  *output = (char*) malloc(frame.size() + 1);
...
static ws_decode_result decode_frame_hybi17(const char* buffer_begin,
  ...
  *output = (char*) malloc(payload_length + 1); // Should be zero-terminated
  if (masked) {
...
static void invoke_read_callback(inspector_socket_t* inspector,
  ...
  if (inspector->ws_state->read_cb) {
    inspector->ws_state->read_cb((uv_stream_t*) &inspector->client,
                                 status, buf);
  }
}
...
static void on_close_frame_written(uv_write_t* write, int status) {
  inspector_socket_t* inspector =
      (inspector_socket_t*) write->handle->data;
  write_request_cleanup(write, status);
  inspector->ws_state->sent_close = true;
...
static void close_frame_received(inspector_socket_t* inspector) {
  ...
  if (!inspector->ws_state->sent_close) {
    const char CLOSE_FRAME[] = {'\x88', '\x00'};
...
static int parse_ws_frames(inspector_socket_t* inspector, size_t len) {
  ...
  if (compressed || r == FRAME_ERROR) {
    ...
    if (r == FRAME_OK && inspector->ws_state->alloc_cb
        && inspector->ws_state->read_cb) {
      uv_buf_t buffer;
      size_t len = strlen(output) + 1;
      inspector->ws_state->alloc_cb((uv_handle_t*) &inspector->client,
          len, &buffer);
...
static void prepare_buffer(uv_handle_t* stream, size_t len, uv_buf_t* buf) {
  ...
  if (len > (inspector->buffer_size - inspector->data_len)) {
    ...
    inspector->buffer = (char*) realloc(inspector->buffer,
...
void inspector_read_stop(inspector_socket_t* inspector) {
  uv_read_stop((uv_stream_t*) &inspector->client);
...
static void generate_accept_string(const char* clientKey, char* buffer) {
  ...
  int keyLen = strlen(clientKey);

  char* buf = (char*) malloc(keyLen + sizeof(wsMagic));
  CHECK_NE(buf, nullptr);
  strcpy(buf, clientKey);
  strcpy(buf + keyLen, wsMagic);

  unsigned char hash[20];

  SHA1((unsigned char*) buf, keyLen + sizeof(wsMagic) - 1, hash);
  free(buf);
...
static void append(char** value, const char* string, size_t length) {
  ...
  *value = (char*) realloc(*value, (new_len / INCREMENT + 1) * INCREMENT);
...
static int header_value_cb(http_parser* parser, const char* at, size_t length) {
  ...
  struct http_parsing_state_s* state = (struct http_parsing_state_s*)
      ((inspector_socket_t*) parser->data)->http_parsing_state;
...
static int message_complete_cb(http_parser* parser) {
  inspector_socket_t* inspector = (inspector_socket_t*) parser->data;
...
static void handshake_data_cb(inspector_socket_t* inspector,
                              ssize_t read, const uv_buf_t* data) {
  http_parsing_state_s* state = inspector->http_parsing_state;
...
int inspector_accept(uv_stream_t* server, inspector_socket_t* inspector,
  ...
  uv_stream_t* client = (uv_stream_t*) &inspector->client;
...
static void close_frame_written_cb(uv_write_t* req, int status) {
  inspector_socket_t* inspector =
      (inspector_socket_t*) req->handle->data;
  inspector->ws_state->sent_close = true;
...
void inspector_close(inspector_socket_t* inspector,
  ...
  // need to do the same.
  ASSERT(!uv_is_closing((uv_handle_t*) &inspector->client));
  ...
  if (inspector->ws_state->sent_close) {
    ...
    const char CLOSE_FRAME[] = {'\x88', '\x00'};
    write_to_client(inspector, CLOSE_FRAME, sizeof(CLOSE_FRAME),
                    close_frame_written_cb);
  }
}

bool inspector_is_active(const struct inspector_socket_s* inspector) {
  return !inspector->shutting_down
         && !uv_is_closing((uv_handle_t*) &inspector->client);
}
